...
```
Google Calendar ──> найближча 1:1 ──> нагадування за добу (з минулих фоллоуапів)
       │                              ├─> ведучому: повний бриф (Telegram)
       │                              ├─> учаснику: лише його частина (Telegram)
       │                              └─> нотатки події: спільний порядок денний (Calendar)
...
5. Для **майбутньої** 1:1, за добу до неї (робочі дні), виходить нагадування (зібране з
   минулих фоллоуапів): **ведучий** отримує повний бриф (усе відкрите + що
   підняти), а **учасник** — лише свою частину. Той самий набір даних також
   пишеться в нотатки самої події календаря — спільний порядок денний, який
   обидва бачать без Telegram (див. `CLAUDE.md`, Rule 0).
...
- **нагадування за добу** — лише свою частину (його домовленості й що на ньому
...
- **OAuth-клієнт** (Desktop) → `secrets/google-oauth-client.json`. Скоупи:
  `calendar.events` (читання й запис — бот пише порядок денний у нотатки
  самої події, а не лише читає зустрічі), `gmail.readonly`, `spreadsheets`,
  `tasks`.
...
Перезапускайте це (локально й для секрету `GOOGLE_TOKEN_JSON` на сервері)
щоразу, коли в `GOOGLE_SCOPES` додається новий скоуп — команда форсує
повторну згоду й перезаписує токен, тож старий токен не продовжить тихо
працювати з меншими правами, ніж тепер потрібно коду.
...
python -m app.cli --job followups                      # лише розіслати нагадування за добу
